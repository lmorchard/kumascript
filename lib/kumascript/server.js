...
var Server = ks_utils.Class({
    ...
    docs_GET: function (req, res) {
        ...
        // Set the Vary: header no matter what, because caching needs it to
        // work out how to build a cache key.
        res.header('Vary', 'X-FireLogger');
    ...
    docs_POST: function (req, res) {
        ...
        form.parse(req, function(err, fields, files) {
            ...
            var accept_data = function (data, name) {
                if ('_document' == name) {
                    src = data;
                } else {
                    templates[name] = data;
            ...
            _.each(fields, accept_data);
            _.each(files, accept_data);

            var WrappedTemplateLoader = ks_utils.Class(ks_loaders.BaseLoader, {
                initialize: function (options) {
                    ks_loaders.BaseLoader.prototype.initialize.apply(this, arguments);
                    this.loader = new options.loader_class(options.loader_options);
                    this.templates = options.templates;
                },
                load: function (name, loaded_cb) {
                    var $this = this,
                        tmpl_source = this.templates[name],
                        cls = this.options.loader_options.template_class;
                    if (tmpl_source) {
                        loaded_cb(null, new cls({ source: tmpl_source }));
                    } else {
                        $this.loader.load(name, loaded_cb);
                    }
                }
            });

            var macro_processor = new ks_macros.MacroProcessor({
                loader_class: WrappedTemplateLoader,
                loader_options: {
                    templates: templates,
                    loader_class: $this.macro_processor.options.loader_class,
                    loader_options: $this.macro_processor.options.loader_options
                }
            });

            var api_ctx = new ks_api.APIContext({
                request: req, response: res, source: src
            });
    ...
    _evalMacros: function (src, req, res) {
        ...
        try {
            $this.macro_processor.process(src, api_ctx, function (errors, result) {
                if (errors) {
                    errors.forEach(function (error) {
                        res.log.error(error.message, { 
                            name: 'kumascript',
